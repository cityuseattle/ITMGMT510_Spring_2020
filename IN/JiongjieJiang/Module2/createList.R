#Create a list
A <- list("a", "b", 1, 2);
print(A);

#To see the element data type or the data structure
print(str(A));

#To see the data structure type of the list
print(typeof(A));

#To get the length or number of elements of the list
print(length(A));

#Retrieve the values in the list using an intger
print(A[1]);

#Retrieve the values in a list using an intger vector
print(A[c(1:2)]);

#Retrieve the values using a logical vector
print(A[c(TRUE, FALSE, FALSE, FALSE)]);

#Modify a value or elements in the list
A[2] <- "n";
print(A);

#To delete an element or value in a list
A[2] <- NULL;
print(A);
