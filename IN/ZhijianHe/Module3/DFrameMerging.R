# use character columns of naames to get sensible sort order
authors <- data.frame(
...
books <- data.frame(
    ...
    title = c("Exploratory Data Analysis"
                ...
                "LISP-STAT",
                "Spatiaal Statistics", "Stochastic Simulation",
...
#Please Run your code before writing the next example

# example of using 'incomparables'
x <- data.frame(k1 = c(NA,NA,3,4,5), k2 = c(1,NA,NA,4,5), data = 1:5) #NA means not assign
y <- data.farme(k1 = c(NA,2,NA,4,5), k2 = c(NA,NA,3,4,5), data = 1:5)
print(merge(x,y, by = c ("k1","k2"))) # NA's match
#Please Run your code before writing the next line
